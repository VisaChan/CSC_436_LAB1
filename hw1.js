...
const Product = class Product {
    ...
    constructor(input){
        let parsedInputArray = input.split(",");
        this.name = parsedInputArray[0];
        this.price = Number(parsedInputArray[1]);
        this.availability = parsedInputArray[2];
    }
    ...
    static printProducts = (products) => {
        products.forEach(element => {
            let part1 = `Product: ${element.name}`;
            let part2 = `Cost: ${new Intl.NumberFormat('en-US', {style: 'currency', currency: 'USD'}).format(element.price)}`;
            let part3 = `Availability: ${element.availability === "In Stock" ? "Yes" : "No"}`;
